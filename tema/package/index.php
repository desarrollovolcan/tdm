<?php 
	 require_once __DIR__ . '/config/dz.php';
?>
<!DOCTYPE html>
<html lang="en">

<head>
	<title><?php echo !empty($DexignZoneSettings['pagelevel'][$CurrentPage]['title']) ? $DexignZoneSettings['pagelevel'][$CurrentPage]['title'].' | ' : '' ; echo $DexignZoneSettings['site_level']['site_title'] ?></title>

	<?php include 'elements/meta.php';?>

	<!-- FAVICONS ICON -->
	<link rel="shortcut icon" type="image/png" href="<?php echo $DexignZoneSettings['site_level']['favicon']?>">
	<?php include 'elements/page-css.php'; ?>

</head>
<body>

    <!--*******************
        Preloader start
    ********************-->
	<?php include 'elements/preloader.php'; ?>
    <!--*******************
        Preloader end
    ********************-->

    <!--**********************************
        Main wrapper start
    ***********************************-->
    <div id="main-wrapper">

        <!--**********************************
            Nav header start
        ***********************************-->
			<?php include 'elements/nav-header.php'; ?>

        <!--**********************************
            Nav header end
        ***********************************-->
		
		<!--**********************************
            Chat box start
        ***********************************-->
		<?php include 'elements/chatbox.php'; ?>
		<!--**********************************
            Chat box End
        ***********************************-->
		
		<!--**********************************
            Header start
        ***********************************-->
       		<?php include 'elements/header.php'; ?>

                    
        <!--**********************************
            Header end ti-comment-alt
        ***********************************-->

        <!--**********************************
            Sidebar start
        ***********************************-->
		<?php include 'elements/sidebar.php'; ?>
        <!--**********************************
            Sidebar end
        ***********************************-->
		
		<!--**********************************
            Content body start
        ***********************************-->
		<div class="content-body">
            <!-- row -->
			<div class="container-fluid">
				<div class="d-flex align-items-center justify-content-between flex-wrap">
					<div>
						<h3 class="mb-1 font-w600 main-text">Good Morning ,Yatin Xarma </h3>
						<p>Hey What's happing Your Invoice Super Admin.</p>
					</div>
					<div class="mb-sm-0 mb-3">
						<a href="javascript:void(0);" class="btn btn-primary btn-sm me-3" data-bs-toggle="modal" data-bs-target="#exampleModal">New Invoice</a>
						<a href="javascript:void(0);" class="btn btn-primary light btn-sm">TRANSFER</a>

					</div>
				</div>
				<div class="row">
					<div class="col-xl-6">
						<div class="row">
							<div class="col-xl-12">
								<div class="card">
									<div class="card-body">
										<div class="row">
											<div class="col-xl-4 col-sm-4">
												<img src="assets/images/invoice.png" alt="" class="w-75 move-1">
											</div>
											<div class="col-xl-8 col-sm-8">
												<div class="home-content ms-auto">
													<h4 class="card-title">Creating polished invoices effortlessly</h4>
													<p>Efficiently identify your top clients and what drives them to promptly settle their invoices.</p>
													<a href="javascript:void(0);" class="btn btn-primary btn-sm">WATCH VIDEO</a>
												</div>
											</div>																	
										</div>
									</div>
								</div>
							</div>
							<div class="col-xl-6 col-sm-6">
								<div class="card">
									<div class="card-header border-0">
										<div class="invoices">
											<h4 class="card-title font-w600">2,478</h4>
											<span>Unpaid Invoices</span>
										</div>
										<div class="invoice-icon bgl-danger">
											<span>
												<i class="flaticon-project fs-22 text-danger"></i>
											</span>
											
										</div>
									</div>
									<div class="card-body p-0 pt-2">
										<div id="widgetChart4"></div>
									</div>
								</div>
							</div>
							<div class="col-xl-6 col-sm-6">
								<div class="card">
									<div class="card-header border-0">
										<div class="invoices">
											<h4 class="card-title font-w600">2,478</h4>
											<span>Send Invoices</span>
										</div>
										<div class="invoice-icon bgl-info">
											<span>
												<i class="flaticon-project fs-22 text-info"></i>
											</span>
											
										</div>
									</div>
									<div class="card-body p-0 pt-2">
										<div id="widgetChart5"></div>
									</div>
								</div>
							</div>
							
						</div>
					</div>
					<div class="col-xl-6">
						<div class="card">
							<div class="card-header border-0 flex-wrap pb-0">
								<h4 class="card-title">Total Revenue</h4>
								<div class="d-flex align-items-center">
									<div class="me-3 custome-check">
										 <input class="form-check-input" type="checkbox" value="" id="chekbox1">
											  <label class="form-check-label" for="chekbox1">
												Monthly
										</label>
									</div>
									<div class="me-3 custome-check">
										<input class="form-check-input" type="checkbox" value="" id="chekbox2">
										<label class="form-check-label" for="chekbox2">
												Weekly
										</label>
									</div>
									<div class="dropdown">
										<a href="javascript:void(0);" class="btn-link" data-bs-toggle="dropdown" aria-expanded="false">
											<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
												<path d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
												<path d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
												<path d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
											</svg>
										</a>
										<div class="dropdown-menu">
											<a class="dropdown-item" href="javascript:void(0);">Delete</a>
											<a class="dropdown-item" href="javascript:void(0);">Edit</a>
										</div>
									</div>
								</div>
							</div>
							<div class="card-body pb-0 pt-3">
								<div class="d-flex align-items-center flex-wrap justify-content-between">
									<div class="d-flex align-items-center mb-2">
										<div class="me-3">
											<svg width="42" height="42" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
												<circle cx="26" cy="26" r="26" fill="#2BC155"/>
												<g clip-path="url(#clip0)">
												<path d="M34.9293 30.4004C34.9294 30.3966 34.9293 30.3927 34.9293 30.3889L34.9371 18.5152C34.9369 18.4212 34.9264 18.3275 34.906 18.2357L34.8284 17.9716L34.7507 17.8163L34.6653 17.7309C34.5585 17.5759 34.4243 17.4417 34.2693 17.3348L34.1916 17.2572L34.0984 17.164L33.8965 17.1019C33.783 17.0683 33.6654 17.0499 33.547 17.0475L21.6112 17.0708C20.8167 17.0676 20.17 17.7092 20.1668 18.5037C20.1668 18.5075 20.1668 18.5114 20.1668 18.5152C20.1853 19.3009 20.8178 19.9334 21.6035 19.9519L28.6935 19.9596C28.9967 19.9626 29.2402 20.2109 29.2372 20.5141C29.2358 20.6552 29.1802 20.7903 29.0818 20.8915L17.5187 32.4546C16.9568 33.0164 16.9568 33.9273 17.5186 34.4892C18.0804 35.0511 18.9913 35.0511 19.5532 34.4893C19.5533 34.4893 19.5533 34.4892 19.5534 34.4892L31.1164 22.9261C31.3338 22.7147 31.6815 22.7196 31.8929 22.937C31.9912 23.0382 32.0469 23.1733 32.0483 23.3144L32.0405 30.3889C32.0551 31.1805 32.6933 31.8188 33.4849 31.8333C34.2795 31.8365 34.9262 31.195 34.9293 30.4004Z" fill="white"/>
												</g>
												<defs>
												<clipPath>
												<rect width="24" height="24" fill="white" transform="translate(26 9.02945) rotate(45)"/>
												</clipPath>
												</defs>
											</svg>
										</div>
										<div>
											<h4 class="fs-20 font-w600 mb-0">$ 459,234.08</h4>
										</div>
									</div>
									<div>
										<small class="text-success font-w600 d-block text-end">+0.6%</small>
										<p class="mb-0">Than Last week</p>
									</div>
								</div>	
								<div id="revenueMap" class="revenueMap"></div>
							</div>
						</div>
					</div>
				</div>
				<!-- row -->
				<div class="row">	
					<div class="col-xl-6 col-xxl-12">
						<div class="card">	
							<div class="card-header border-0">
								<h4 class="card-title mb-0">Spendings</h4>
							</div>
							<div class="card-body pt-0">
								<div class="row">
									<div class="col-xl-5  col-md-12">
										<div class="row">
											<div class="d-flex col-xl-12  col-sm-6 col-6 mb-4">
												<svg class="me-2" width="14" height="54" viewBox="0 0 14 54" fill="none" xmlns="http://www.w3.org/2000/svg">
													<rect width="10" height="50" rx="7" fill="url(#paint0_linear_15_1435)"/>
													<defs>
													<linearGradient id="paint0_linear_15_1435" x1="17.6556" y1="9.43374" x2="-2.50316" y2="14.2536" gradientUnits="userSpaceOnUse">
													<stop offset="0" stop-color="#AC39D5"/>
													<stop offset="1" stop-color="#D539C5"/>
													</linearGradient>
													</defs>
												</svg>

												<div>
													<p class="fs-14 mb-0">Investment</p>
													 <span class="fs-16 font-w600 text-gray">
													 <span class="text-black me-2 fs-18 font-w700">$1,567</span>/$5,000
													</span>
												</div>
											</div>
											<div class="d-flex col-xl-12  col-sm-6 col-6 mb-4">
												<svg class="me-2" width="14" height="54" viewBox="0 0 14 54" fill="none" xmlns="http://www.w3.org/2000/svg">
													<rect width="10" height="50" rx="7" fill="url(#paint0_linear_15_1436)"/>
													<defs>
													<linearGradient id="paint0_linear_15_1436" x1="7" y1="0" x2="-20.413" y2="15.8834" gradientUnits="userSpaceOnUse">
													<stop offset="0" stop-color="#40D5A8"/>
													<stop offset="0.993633" stop-color="#40BAD5"/>
													</linearGradient>
													</defs>
												</svg>

												<div>
													<p class="fs-14 mb-0">Installment</p>
													<span class="fs-16 font-w600 text-gray"><span class="text-black me-2 fs-18 font-w700">$1,567</span>/$5,000</span>
												</div>
											</div>
											<div class="d-flex col-xl-12 col-xxl-6 col-sm-6 col-6 mb-4">
												<svg class="me-2" width="14" height="54" viewBox="0 0 14 54" fill="none" xmlns="http://www.w3.org/2000/svg">
													<rect width="10" height="50" rx="7" fill="url(#paint0_linear_15_1438)"/>
													<defs>
													<linearGradient id="paint0_linear_15_1438" x1="7" y1="5.51899e-07" x2="-21.5754" y2="21.6028" gradientUnits="userSpaceOnUse">
													<stop offset="0" stop-color="#461EE7"/>
													<stop offset="1" stop-color="#1EB6E7"/>
													</linearGradient>
													</defs>
												</svg>

												<div>
													<p class="fs-14 mb-0">Property</p>
													<span class="fs-16 font-w600 text-gray"><span class="text-black me-2 18 font-w700">$1,567</span>/$5,000</span>
												</div>
											</div>
											<div class="d-flex col-xl-12  col-sm-6 col-6 mb-4">
												<svg class="me-2" width="14" height="54" viewBox="0 0 14 54" fill="none" xmlns="http://www.w3.org/2000/svg">
													<rect width="10" height="50" rx="7" fill="url(#paint0_linear_15_1437)"/>
													<defs>
													<linearGradient id="paint0_linear_15_1437" x1="11.55" y1="-2.27711" x2="-13.7615" y2="5.22803" gradientUnits="userSpaceOnUse">
													<stop offset="0" stop-color="#FF827A"/>
													<stop offset="1" stop-color="#FFA825"/>
													</linearGradient>
													</defs>
												</svg>

												<div>
													<p class="fs-14 mb-0">Restaurant</p>
													<span class="fs-16 font-w600 text-gray"><span class="text-black me-2 fs-18 font-w700">$1,567</span>/$5,000</span>
												</div>
											</div>
										
										</div>
									</div>
									<div class="col-xl-7 col-md-12">
										<div class="row">
											<div class="col-sm-6 mb-4 col-6">
												<div class="bg-gradient1 rounded text-center p-3  pt-4">
													<div class="d-inline-block position-relative donut-chart-sale mb-3">
														<span class="donut1" data-peity='{ "fill": ["rgb(255, 255, 255)", "rgba(255, 255, 255, 0.2)"],   "innerRadius": 33, "radius": 10}'>5/8</span>
														<small class="text-white">71%</small>
													</div>
													<span class="fs-16 font-w600 text-white d-block">Investment</span>
												</div>
											</div>
											<div class="col-sm-6 mb-4 col-6">
												<div class="bg-gradient2 rounded text-center p-3 pt-4">
													<div class="d-inline-block position-relative donut-chart-sale mb-3">
														<span class="donut1" data-peity='{ "fill": ["rgb(255, 255, 255)", "rgba(255, 255, 255, 0.2)"],   "innerRadius": 33, "radius": 10}'>3/8</span>
														<small class="text-white">30%</small>
													</div>
													<span class="fs-16 font-w600 text-white d-block">Installment</span>
												</div>
											</div>
											<div class="col-sm-6 mb-sm-0 mb-4 col-6">
												<div class="rounded text-center p-3 bg-gradient3  pt-4">
													<div class="d-inline-block position-relative donut-chart-sale mb-3">
														<span class="donut1" data-peity='{ "fill": ["rgb(255, 255, 255)", "rgba(234, 234, 234, 0.2)"],   "innerRadius": 33, "radius": 10}'>1/8</span>
														<small class="text-white">5%</small>
													</div>
													<span class="fs-16 font-w600 text-white d-block">Restaurant</span>
												</div>
											</div>
											<div class="col-sm-6 mb-sm-0 mb-4 col-6">
												<div class="rounded text-center p-3 bg-gradient4  pt-4">
													<div class="d-inline-block position-relative donut-chart-sale mb-3">
														<span class="donut1" data-peity='{ "fill": ["rgb(255, 255, 255)", "rgba(255, 255, 255, 0.2)"],   "innerRadius": 33, "radius": 10}'>9/10</span>
														<small class="text-white">96%</small>
													</div>
													<span class="fs-16 font-w600 text-white d-block">Property</span>
												</div>
											</div>
										</div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-xl-3 col-xxl-6 col-sm-6">
						<div class="card h-auto">
							<div class="card-header border-0 pb-0 flex-wrap">
								<div>
									<h5 class="card-title mb-3">Withdrawn</h5>
									<h4 class="fs-20 font-w600">4,563</h4>
									<span><small class="text-success">+1.6% </small>than last week</span>
								</div>
								<svg width="42" height="42" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
									<rect width="52" height="52" rx="26" fill="#FD5353"/>
									<g clip-path="url(#clip0)">
									<path d="M17.0704 21.5996C17.0704 21.6034 17.0704 21.6073 17.0704 21.6111L17.0626 33.4848C17.0629 33.5788 17.0733 33.6725 17.0937 33.7643L17.1713 34.0284L17.249 34.1837L17.3344 34.2691C17.4413 34.4241 17.5755 34.5583 17.7305 34.6652L17.8081 34.7428L17.9013 34.836L18.1032 34.8981C18.2168 34.9317 18.3343 34.9501 18.4527 34.9525L30.3885 34.9292C31.183 34.9324 31.8297 34.2908 31.8329 33.4963C31.8329 33.4925 31.8329 33.4886 31.8329 33.4848C31.8144 32.6991 31.1819 32.0666 30.3962 32.0481L23.3062 32.0404C23.003 32.0374 22.7595 31.7891 22.7625 31.4859C22.7639 31.3448 22.8195 31.2097 22.9179 31.1085L34.481 19.5454C35.0429 18.9836 35.043 18.0727 34.4811 17.5108C33.9193 16.9489 33.0084 16.9489 32.4465 17.5107C32.4465 17.5107 32.4464 17.5108 32.4464 17.5108L20.8833 29.0739C20.6659 29.2853 20.3182 29.2804 20.1068 29.063C20.0085 28.9618 19.9528 28.8267 19.9514 28.6856L19.9592 21.6111C19.9447 20.8195 19.3064 20.1812 18.5148 20.1667C17.7202 20.1635 17.0735 20.805 17.0704 21.5996Z" fill="white"/>
									</g>
									<defs>
									<clipPath >
									<rect width="24" height="24" fill="white" transform="translate(25.9997 42.9706) rotate(-135)"/>
									</clipPath>
									</defs>
								</svg>
							</div>	
							<div class="card-body py-2">
								<div id="chartBarRunning" class="chartBarRunning"></div>
							</div>
						</div>	
					</div>
					<div class="col-xl-3 col-xxl-6 col-sm-6">
						<div class="card">
							<div class="card-header border-0 pb-0">
								<div>
									<h4 class="card-title">Quick Transfer</h4>
									<span>Lorem ipsum dolor sit amet.</span>
								</div>
								<div class="dropdown">
									<a href="javascript:void(0);" class="btn-link" data-bs-toggle="dropdown" aria-expanded="false">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
											<path d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
											<path d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
										</svg>
									</a>
									<div class="dropdown-menu dropdown-menu-right">
										<a class="dropdown-item" href="javascript:void(0);">Delete</a>
										<a class="dropdown-item" href="javascript:void(0);">Edit</a>
									</div>
								</div>
							</div>
							<div class="card-body">	
								<div class="user-bx">
									<img src="assets/images/profile/small/pic1.jpg" alt="">
									<div>
										<h6 class="card-title--small mb-0">Samuel</h6>
										<small>@sam224</small>
									</div>
									<i class="las la-check-circle check-icon"></i>
								</div>
								<h4 class="card-title--small mt-4 font-w600">Recent Friend<a href="javascript:void(0);" class="float-end text-primary fs-13">See More</a></h4>
								<ul class="user-list">
									<li><img src="assets/images/avatar/1.jpg" alt=""></li>
									<li><img src="assets/images/avatar/2.jpg" alt=""></li>
									<li><img src="assets/images/avatar/3.jpg" alt=""></li>
									<li><img src="assets/images/avatar/4.jpg" alt=""></li>
									<li><img src="assets/images/avatar/5.jpg" alt=""></li>
									<li><img src="assets/images/avatar/6.jpg" alt=""></li>
									<li><img src="assets/images/avatar/1.jpg" alt=""></li>
									<li><img src="assets/images/avatar/1.jpg" alt=""></li>
									
									
								</ul>
								<h4 class="card-title--small mt-3 font-w600">Insert Amount</h4>
								<div class="format-slider">
									<input class="form-control amount-input"  title="Formatted number" id="input-format">
									<div id="slider-format"></div>
								</div>
								<div class="fs-16 d-flex justify-content-between font-w600 mt-3">
									<span class="text-grey">Your Balance</span>
									<span class="text-black">$ 456,345.62</span>
								</div>
							</div>
							<div class="card-footer border-0 pt-0">
								<a href="javascript:void(0);" class="btn btn-primary d-block  text-uppercase">Transfer Now</a>
							</div>
						</div>
					</div>
					
				</div>
				<div class="row">
					<div class="col-xl-5">
						<div class="card overflow-hidden">
							<div class="card-header d-sm-flex d-block border-0 pb-0">
								<div class="mb-sm-0 mb-2">
									<p>Weekly Wallet Usage</p>
									<span class="mb-0">
									<svg width="12" height="6" viewBox="0 0 12 6" fill="none" xmlns="http://www.w3.org/2000/svg">
										<path d="M11.9999 6L5.99994 -2.62268e-07L-6.10352e-05 6" fill="#2BC155"/>
									</svg>
									<strong class="fs-26 text-black ms-2 me-3">43%</strong>Than last week</span>
								</div>
								<span class="fs-12">
								<svg width="21" height="15" viewBox="0 0 21 15" fill="none" xmlns="http://www.w3.org/2000/svg">
									<path d="M0.999939 13.5C1.91791 12.4157 4.89722 9.22772 6.49994 7.5L12.4999 10.5L19.4999 1.5" stroke="#2BC155" stroke-width="2"/>
									<path d="M6.49994 7.5C4.89722 9.22772 1.91791 12.4157 0.999939 13.5H19.4999V1.5L12.4999 10.5L6.49994 7.5Z" fill="url(#paint0_linear2)"/>
									<defs>
									<linearGradient id="paint0_linear2" x1="10.2499" y1="3" x2="10.9999" y2="13.5" gradientUnits="userSpaceOnUse">
									<stop offset="0" stop-color="#2BC155" stop-opacity="0.73"/>
									<stop offset="1" stop-color="#2BC155" stop-opacity="0"/>
									</linearGradient>
									</defs>
								</svg>
								4% (30 days)</span>
							</div>
							<div class="card-body p-0">
								<canvas id="widgetChart3" height="80"></canvas>
							</div>
						</div>
					</div>
					<div class="col-xl-7">
						<div class="card">
							<div class="card-header border-0 pb-0">
								<h4 class="card-title">Top Sales</h4>
							</div>
							<div class="card-body">
								<div class="row">
									<div class="col-xl-3 col-md-3">
										<div class="dz-sales-bx bgl-primary mb-md-0 mb-3">
											<i class="flaticon-app text-primary fs-24 mb-2"></i>
											<h4 class="card-title--small mb-0">Invoice Status</h4>
											<small>1000 sales</small>
										</div>
									</div>
									<div class="col-xl-3 col-md-3">
										<div class="dz-sales-bx bgl-success mb-md-0 mb-3">
											<i class="flaticon-bar-chart text-success fs-24 mb-2"></i>
											<h4 class="card-title--small mb-0">Trends  Status</h4>
											<small>1000 sales</small>
										</div>
									</div>
									<div class="col-xl-3 col-md-3">
										<div class="dz-sales-bx bgl-info">
											<i class="flaticon-user text-info fs-24 mb-2"></i>
											<h4 class="card-title--small mb-0">Cus. Status</h4>
											<small>1000 sales</small>
										</div>
									</div>
									<div class="col-xl-3 col-md-3">
										<div class="dz-sales-bx bgl-danger">
											<i class="flaticon-app text-danger fs-24 mb-2"></i>
											<h4 class="card-title--small mb-0">Performance Status</h4>
											<small>1000 sales</small>
										</div>
									</div>
								</div>
							</div> 
						</div>
					</div>
				</div>
				<!-- row -->
				<div class="row">
					<div class="col-xl-6">
						<div class="card">
							<div class="card-header d-block d-sm-flex border-0 flex-wrap transactions-tab">
								<div class="me-3 mb-3">
									<h4 class="card-title">Previous Transactions</h4>
									<span>Lorem ipsum dolor sit amet, consectetur</span>
								</div>
								<div class="card-tabs mt-3 mt-sm-0 mb-3 ">
									<ul class="nav nav-tabs" role="tablist">
										<li class="nav-item">
											<a class="nav-link active" data-bs-toggle="tab" href="#monthly" role="tab">Monthly</a>
										</li>
										<li class="nav-item">
											<a class="nav-link" data-bs-toggle="tab" href="#Weekly" role="tab">Weekly</a>
										</li>
										<li class="nav-item">
											<a class="nav-link" data-bs-toggle="tab" href="#Today" role="tab">Today</a>
										</li>
									</ul>
								</div>
							</div>
							<div class="card-body tab-content p-0">
								<div class="tab-pane active show fade" id="monthly" role="tabpanel">
									<div class="table-responsive">
										<table class="table table-responsive-md card-table transactions-table">
											<tbody>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#FF2E2E"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Chef Renata</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="font-w600 mb-0">June 5, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">-$167</span></td>
													<td><span class="text-warning  text-end d-block">Pending</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#09BD3C"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>

													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">XYZ Store ID</a></h6>
														<small>Cashback</small>
													</td>
													<td>
														<h6 class="font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="font-w600 text-dark">+$5,553</span></td>
													<td><span class="text-success text-end d-block">Completed</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#09BD3C"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>

													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">XYZ Store ID</a></h6>
														<small>Cashback</small>
													</td>
													<td>
														<h6 class="font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="font-w600 text-dark">+$5,553</span></td>
													<td><span class="text-success text-end d-block">Completed</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#FF2E2E"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Hawkins Jr.</a></h6>
														<small>Cashback</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-danger text-end d-block">Canceled</span></td>
												</tr>
											</tbody>
										</table>
									</div>
								</div>
								<div class="tab-pane" id="Weekly" role="tabpanel">
									<div class="table-responsive">
										<table class="table table-responsive-md card-table transactions-table">
											<tbody>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#09BD3C"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">XYZ Store ID</a></h6>
														<small>Cashback</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-success  text-end d-block">Completed</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#FF2E2E"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Chef Renata</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 5, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">-$167</span></td>
													<td><span class="text-warning  text-end d-block">Pending</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#09BD3C"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Cindy Alexandro</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 5, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-danger  text-end d-block">Canceled</span></td>
												</tr>
											</tbody>
										</table>
									</div>
								</div>
								<div class="tab-pane" id="Today" role="tabpanel">
									<div class="table-responsive">
										<table class="table table-responsive-md card-table transactions-table">
											<tbody>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#09BD3C"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Chef Renata</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 5, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">-$167</span></td>
													<td><span class="text-warning font-w600 text-end d-block">Pending</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect width="63" height="63" rx="30" fill="#FF2E2E"/>
															<g clip-path="url(#clip0_6_69)">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="white"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8813 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6274 37.6008 29.1297 37.1031L25.6899 33.6633C25.1539 33.1274 25.1539 32.2633 25.6899 31.7274C26.2258 31.1914 27.0899 31.1914 27.6258 31.7274L29.6438 33.7453L29.6438 25.9742C29.6438 25.2196 30.2563 24.6071 31.011 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="white"/>
															</g>
															<defs>
															<clipPath>
															<rect width="28" height="28" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 17 17)"/>
															</clipPath>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Cindy Alexandro</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 5, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-danger  text-end d-block">Canceled</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect x="1" y="1" width="61" height="61" rx="29" stroke="#FF2E2E" stroke-width="2"/>
															<g clip-path="">
															<path d="M35.2219 19.0125C34.8937 19.6906 35.1836 20.5109 35.8617 20.8391C37.7484 21.7469 39.3453 23.1578 40.4828 24.9242C41.6476 26.7344 42.2656 28.8344 42.2656 31C42.2656 37.2125 37.2125 42.2656 31 42.2656C24.7875 42.2656 19.7344 37.2125 19.7344 31C19.7344 28.8344 20.3523 26.7344 21.5117 24.9187C22.6437 23.1523 24.2461 21.7414 26.1328 20.8336C26.8109 20.5055 27.1008 19.6906 26.7726 19.007C26.4445 18.3289 25.6297 18.0391 24.9461 18.3672C22.6 19.4937 20.6148 21.2437 19.2094 23.4422C17.7656 25.6953 17 28.3094 17 31C17 34.7406 18.4547 38.257 21.1015 40.8984C23.743 43.5453 27.2594 45 31 45C34.7406 45 38.257 43.5453 40.8984 40.8984C43.5453 38.2516 45 34.7406 45 31C45 28.3094 44.2344 25.6953 42.7851 23.4422C41.3742 21.2492 39.389 19.4937 37.0484 18.3672C36.3648 18.0445 35.55 18.3289 35.2219 19.0125Z" fill="#FF2E2E"/>
															<path d="M36.3211 30.2726C36.589 30.0047 36.7203 29.6547 36.7203 29.3047C36.7203 28.9547 36.589 28.6047 36.3211 28.3367L32.8812 24.8969C32.3781 24.3937 31.7109 24.1203 31.0055 24.1203C30.3 24.1203 29.6273 24.3992 29.1297 24.8969L25.6898 28.3367C25.1539 28.8726 25.1539 29.7367 25.6898 30.2726C26.2258 30.8086 27.0898 30.8086 27.6258 30.2726L29.6437 28.2547L29.6437 36.0258C29.6437 36.7804 30.2562 37.3929 31.0109 37.3929C31.7656 37.3929 32.3781 36.7804 32.3781 36.0258L32.3781 28.2492L34.3961 30.2672C34.9211 30.8031 35.7851 30.8031 36.3211 30.2726Z" fill="#FF2E2E"/>
															</g>
															<defs>
															</defs>
														</svg>
													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Paipal</a></h6>
														<small>Transfer</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-success text-end d-block">Completed</span></td>
												</tr>
												<tr>
													<td>
														<svg width="53" height="53" viewBox="0 0 63 63" fill="none" xmlns="http://www.w3.org/2000/svg">
															<rect x="1.00002" y="1" width="61" height="61" rx="29" stroke="#2BC155" stroke-width="2"/>
															<g clip-path="">
															<path d="M35.2219 42.9875C34.8938 42.3094 35.1836 41.4891 35.8617 41.1609C37.7484 40.2531 39.3453 38.8422 40.4828 37.0758C41.6477 35.2656 42.2656 33.1656 42.2656 31C42.2656 24.7875 37.2125 19.7344 31 19.7344C24.7875 19.7344 19.7344 24.7875 19.7344 31C19.7344 33.1656 20.3523 35.2656 21.5117 37.0813C22.6437 38.8477 24.2461 40.2586 26.1328 41.1664C26.8109 41.4945 27.1008 42.3094 26.7727 42.993C26.4445 43.6711 25.6297 43.9609 24.9461 43.6328C22.6 42.5063 20.6148 40.7563 19.2094 38.5578C17.7656 36.3047 17 33.6906 17 31C17 27.2594 18.4547 23.743 21.1016 21.1016C23.743 18.4547 27.2594 17 31 17C34.7406 17 38.257 18.4547 40.8984 21.1016C43.5453 23.7484 45 27.2594 45 31C45 33.6906 44.2344 36.3047 42.7852 38.5578C41.3742 40.7508 39.3891 42.5063 37.0484 43.6328C36.3648 43.9555 35.55 43.6711 35.2219 42.9875Z" fill="#2BC155"/>
															<path d="M36.3211 31.7274C36.5891 31.9953 36.7203 32.3453 36.7203 32.6953C36.7203 33.0453 36.5891 33.3953 36.3211 33.6633L32.8812 37.1031C32.3781 37.6063 31.7109 37.8797 31.0055 37.8797C30.3 37.8797 29.6273 37.6008 29.1297 37.1031L25.6898 33.6633C25.1539 33.1274 25.1539 32.2633 25.6898 31.7274C26.2258 31.1914 27.0898 31.1914 27.6258 31.7274L29.6437 33.7453L29.6437 25.9742C29.6437 25.2196 30.2562 24.6071 31.0109 24.6071C31.7656 24.6071 32.3781 25.2196 32.3781 25.9742L32.3781 33.7508L34.3961 31.7328C34.9211 31.1969 35.7852 31.1969 36.3211 31.7274Z" fill="#2BC155"/>
															</g>
															<defs>
															</defs>
														</svg>



													</td>
													<td>
														<h6 class="font-w600 mb-0"><a href="javascript:void(0);" class="text-black">Hawkins Jr.</a></h6>
														<small>Cashback</small>
													</td>
													<td>
														<h6 class="text-black font-w600 mb-0">June 4, 2020</h6>
														<small>05:34:45 AM</small>
													</td>
													<td><span class="text-black font-w600">+$5,553</span></td>
													<td><span class="text-danger text-end d-block">Canceled</span></td>
												</tr>
											</tbody>
										</table>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="col-xl-3 col-sm-6">
						<div class="card">
							<div class="card-header border-0 pb-0">
								<div>
									<h4 class="card-title">Bar Spendings</h4>
									<span>Lorem ipsum dolor sit amet.</span>
								</div>
								<div class="dropdown">
									<a href="javascript:void(0);" class="btn-link" data-bs-toggle="dropdown" aria-expanded="false">
										<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M12 13C12.5523 13 13 12.5523 13 12C13 11.4477 12.5523 11 12 11C11.4477 11 11 11.4477 11 12C11 12.5523 11.4477 13 12 13Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
											<path d="M12 6C12.5523 6 13 5.55228 13 5C13 4.44772 12.5523 4 12 4C11.4477 4 11 4.44772 11 5C11 5.55228 11.4477 6 12 6Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
											<path d="M12 20C12.5523 20 13 19.5523 13 19C13 18.4477 12.5523 18 12 18C11.4477 18 11 18.4477 11 19C11 19.5523 11.4477 20 12 20Z" stroke="#575757" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
										</svg>
									</a>
									<div class="dropdown-menu dropdown-menu-right">
										<a class="dropdown-item" href="javascript:void(0);">Delete</a>
										<a class="dropdown-item" href="javascript:void(0);">Edit</a>
									</div>
								</div>
							</div>
							<div class="card-body dz-scroll dz-spending">	
								<div class="progress default-progress">
									<div class="progress-bar bg-gradient1 progress-animated" style="width: 45%; height:10px;" role="progressbar">
										<span class="sr-only">45% Complete</span>
									</div>
								</div>
								<div class="d-flex align-items-end mt-2 pb-3 justify-content-between">
									<span>Investment</span>
									<span class="font-w600 text-grey"><span class="text-black pe-2">$1415</span>/$2000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient2 progress-animated" style="width: 70%; height:10px;" role="progressbar">
										<span class="sr-only">70% Complete</span>
									</div>
								</div>
								<div class="d-flex align-items-end mt-2 pb-3 justify-content-between">
									<span>Restaurant</span>
									<span class="font-w600 text-grey"><span class="text-black pe-2">$1567</span>/$5000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient4 progress-animated" style="width: 35%; height:10px;" role="progressbar">
										<span class="sr-only">35% Complete</span>
									</div>
								</div>
								<div class="d-flex align-items-end mt-2 pb-3 justify-content-between">
									<span>Installment</span>
									<span class="font-w600 text-gray"><span class="text-black pe-2">$487</span>/$10000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient3 progress-animated" style="width: 95%; height:10px;" role="progressbar">
										<span class="sr-only">95% Complete</span>
									</div>
								</div>
								
								<div class="d-flex align-items-end mt-2 justify-content-between">
									<span>Property</span>
									<span class="font-w600 text-grey"><span class="text-black pe-2">$3890</span>/$4000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient2 progress-animated" style="width: 70%; height:10px;" role="progressbar">
										<span class="sr-only">70% Complete</span>
									</div>
								</div>
								<div class="d-flex align-items-end mt-2 pb-3 justify-content-between">
									<span>Restaurant</span>
									<span class="font-w600 text-grey"><span class="text-black pe-2">$1567</span>/$5000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient4 progress-animated" style="width: 35%; height:10px;" role="progressbar">
										<span class="sr-only">35% Complete</span>
									</div>
								</div>
								<div class="d-flex align-items-end mt-2 pb-3 justify-content-between">
									<span>Installment</span>
									<span class="font-w600 text-gray"><span class="text-black pe-2">$487</span>/$10000</span>
								</div>
								<div class="progress default-progress mt-4">
									<div class="progress-bar bg-gradient3 progress-animated" style="width: 95%; height:10px;" role="progressbar">
										<span class="sr-only">95% Complete</span>
									</div>
								</div>
								
								<div class="d-flex align-items-end mt-2 justify-content-between">
									<span>Property</span>
									<span class="font-w600 text-grey"><span class="text-black pe-2">$3890</span>/$4000</span>
								</div>
								
							</div>
							<div class="card-footer border-0 pt-0">
								<a href="javascript:void(0);" class="btn btn-outline-primary  d-block">View More</a>
							</div>
						</div>
					</div>
					<div class="col-xl-3 col-sm-6">
						<div class="card">
							<div class="card-header d-sm-flex d-block border-0 pb-0">
								<div class="pe-3 me-auto mb-sm-0 mb-3">
									<h4 class="card-title">Transaction Overview</h4>
									<span>Lorem ipsum dolor sit amet, consectetur</span>
								</div>
							</div>
							<div class="card-body">
								<div id="chartBar" class="chartBar"></div>
							</div>
						</div>
					</div>
				</div>
			</div>
        </div>
	
        <!--**********************************
            Content body end
        ***********************************-->
		<!-- Button trigger modal -->
			
			
			<!-- Modal -->
			<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<div class="modal-header">
					<h1 class="modal-title fs-5" id="exampleModalLabel">New Invoice</h1>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
					</div>
					<div class="modal-body">
						<div class="row">
							<div class="col-xl-12">
								<div class="mb-3">
									<label for="exampleFormControlInput1" class="form-label required">Invoice Id</label>
									<input type="text" class="form-control" id="exampleFormControlInput1" placeholder="#123">
								  </div>
							</div>
						</div>
					</div>
					<div class="modal-footer">
					<button type="button" class="btn btn-danger light" data-bs-dismiss="modal">Close</button>
					<button type="button" class="btn btn-primary">Save changes</button>
					</div>
				</div>
				</div>
			</div>
		
		
        <!--**********************************
            Footer start
        ***********************************-->
        <?php include 'elements/footer.php'; ?>
        <!--**********************************
            Footer end	
        ***********************************-->

		<!--**********************************
           Support ticket button start
        ***********************************-->

        <!--**********************************
           Support ticket button end
        ***********************************-->
			


	</div>
    <!--**********************************
        Main wrapper end
    ***********************************-->

    <!--**********************************
        Scripts
    ***********************************-->
	
    <!-- Required vendors -->
 <?php include 'elements/page-js.php'; ?>
	
    
	<script>
		//Number formatting
		var sliderFormat = document.getElementById('slider-format');
		noUiSlider.create(sliderFormat, {
			start: [20000],
			step: 1000,
			connect: [true, false],
			range: {
				'min': [20000],
				'max': [80000]
			},
			ariaFormat: wNumb({
				decimals: 3
			}),
			format: wNumb({
				decimals: 3,
				thousand: '.',
			})
		});

		var inputFormat = document.getElementById('input-format');
		sliderFormat.noUiSlider.on('update', function (values, handle) {
			inputFormat.value = values[handle];
		});

		inputFormat.addEventListener('change', function () {
			sliderFormat.noUiSlider.set(this.value);
		});
		jQuery(document).ready(function(){
			setTimeout(function(){
				dzSettingsOptions.version = 'light';
				new dzSettings(dzSettingsOptions);
				setCookie('version','light');
			},1000)
		});
		//Number formatting ^
	

	</script>
	
</body>
</html>