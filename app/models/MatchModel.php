...
class MatchModel extends BaseModel
{
    ...
    {
        $stmt = $this->pdo->prepare('SELECT m.*, p1.name AS player1_name, p2.name AS player2_name FROM matches m JOIN players p1 ON m.player1_id = p1.id JOIN players p2 ON m.player2_id = p2.id WHERE m.id = :id');
        $stmt->execute(['id' => $id]);
    ...
    public function recordResult($matchId, $sets)
    {
        ...
        $player2Sets = 0;
        if (empty($sets)) {
            throw new InvalidArgumentException('Ingresa al menos un set para registrar el resultado.');
        }
        foreach ($sets as $set) {
            [$p1, $p2] = $set;
